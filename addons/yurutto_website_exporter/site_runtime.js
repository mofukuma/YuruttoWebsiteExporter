...
const route=()=>{let path=location.pathname;try{path=decodeURIComponent(path)}catch{}let p=path.slice(cfg.root.length-1);if(!p.startsWith('/'))p='/'+p;if(!p.endsWith('/'))p+='/';return Object.entries(list).find(([,v])=>v.uri===p)?.[0]||Object.keys(list)[0]};
...
window.YWebSite={bind(fn){callback=fn;emit()},initialScene(){return list[route()].scene},scene(path){const found=Object.entries(list).find(([,page])=>page.scene===path);if(!found)return;const [name,page]=found;if(pending){if(pending!==name)return;pending='';apply(name);ready();return}apply(name);ready();address(page,false)}};
// 戻る進むに追従し、最初の表示も同じ経路で整える。
addEventListener('popstate',emit);apply(route());
})();
