...
const TEXTS := {
	...
	"publish_copy": ["Cannot prepare a public file: %s -> %s (%s)", "公開fileを準備できません: %s -> %s (%s)"],
	"exported": ["Exported with the bundled template: %s", "内蔵テンプレートで書き出しました: %s"],
	# 本番公開を止める理由。
	"production_https": ["Use HTTPS for the production base URL: %s", "本番base URLはHTTPSにしてください: %s"],
	"production_site_enabled": ["Enable Yurutto Site for a production export.", "本番ExportではYurutto Siteを有効にしてください。"],
	"production_config_path": ["The production config path is invalid: %s", "公開設定pathが不正です: %s"],
	"production_config_object": ["The production config must be a JSON object: %s", "公開設定はJSON objectにしてください: %s"],
	"production_asset_https": ["Use HTTPS for an external asset: %s", "外部assetはHTTPSにしてください: %s"],
	"production_asset_sri": ["An external asset needs integrity and crossorigin=anonymous: %s", "外部assetにはintegrityとcrossorigin=anonymousが必要です: %s"],
	"production_commerce_mode": ["Set the commerce mode to hosted.", "課金modeはhostedを指定してください。"],
	"production_checkout_hosts": ["Add the HTTPS hosted-checkout origins to checkout_hosts.", "Hosted CheckoutのHTTPS originをcheckout_hostsへ指定してください。"],
	"production_checkout_origin": ["Invalid checkout origin: %s", "Checkout originが不正です: %s"],
	"production_commerce_page": ["A paid site needs a public %s page.", "課金siteには公開%s pageが必要です。"],
	"production_secret": ["A public file contains %s: %s", "公開対象に%sがあります: %s"],
	"production_link_https": ["Use HTTPS for a production LinkButton: %s", "本番LinkButtonはHTTPSにしてください: %s"],
	"production_legal_link": ["Every paid page needs a LinkButton to: %s", "課金siteの全pageから法務pageへLinkButtonを置いてください: %s"],
	"production_checkout_link": ["No LinkButton leads to checkout_hosts.", "checkout_hostsへ進むLinkButtonがありません。"],
	"production_host_size": ["A file exceeds the host limit: %s (%s > %s bytes)", "配信先の一file上限を超えています: %s (%s > %s bytes)"],
	"production_security_write": ["Cannot write the security file: %s", "security fileを書けません: %s"],
	"production_permissions": ["Permissions-Policy is invalid.", "Permissions-Policyが不正です。"],
	# 失敗の見出し。
