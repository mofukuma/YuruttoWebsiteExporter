...
## RichTextLabelを一文字ずつ写す

Godotは一文字を描く直前に、揺れや薄れを計算し終えた位置と色を持っている。
`rich_text_label.cpp`のglyph描画の手前で、その値をそのままDOMへ渡す。
DOM側で位置を計算し直さないので、waveやtornadoの動きも自然に付いてくる。

実測でDOM要素は254個になり、一要素が一文字を持つ。位置は送り幅どおりに並ぶ。

| 見るもの | 値 |
| --- | --- |
| 文字要素 | 254 |
| 先頭 | イ x16 w16 / ー x31 w16 / ハ x46 w16 |

要素の幅は`font_get_glyph_advance`でその字の送り幅に合わせる。広く取ると隣と
ぶつかって並びが崩れる。

RMSEは14.035%のまま動いていない。GodotもBrowserも明るい画素はそれぞれ14920と
13625あり、文字は両方に出ている。差は字形そのもので、飾りの太さや色や大きさが
DOM側へ渡っていないためと見ている。次はTextStateへ太字や色や字の大きさを乗せる。

`tests/fixtures/bbcode`の側はまだ出ない。`page_rich`との違いは箱の高さだけに
見えたが、高さを変えても変わらなかった。ここは別の原因が残っている。
