...
const tool = path.join(root, 'build/prepare_yuruttoweb_preset.cjs'); // 独立preset正規化処理。
const fixture = `[preset.0]\n\nname="Web"\nplatform="Web"\n\n[preset.0.options]\n\ncustom_template/release="/old/machine/template.zip"\nhtml/canvas_resize_policy=0\nhtml/focus_canvas_on_start=true\nyuruttoweb/routing/mode=1\nyuruttoweb/font/matching_webfont=false\nyuruttoweb/font/avoid_canvas_theme_font=false\nyuruttoweb/ogp/frame=27\n`;
...
try {
	...
	assert.equal(once.includes('yuruttoweb/tools/node'), false, 'Node.js設定が残存');
	assert.equal(once.includes('/old/machine'), false, 'machine固有pathが残留');
	assert.match(once, /yuruttoweb\/routing\/mode=1/, 'History選択を上書き');
	assert.match(once, /yuruttoweb\/font\/matching_webfont=false/, 'Web font選択を上書き');
	assert.match(once, /yuruttoweb\/font\/avoid_canvas_theme_font=false/, 'Canvas Theme font設定を上書き');
	assert.match(once, /yuruttoweb\/ogp\/frame=27/, 'OGP撮影frameを上書き');
	childProcess.execFileSync(process.execPath, [tool, work]);
