...
async function control(page, kind) {
	return page.evaluate((type) => {
		const node = document.querySelector(`[data-yweb-kind="${type}"]`);
		if (!node) return null;
		const box = node.getBoundingClientRect();
		const style = getComputedStyle(node);
		return {
			...
			logicalMaxLength: Number(node.dataset.ywebMaxLength || 0),
...
(async () => {
	...
	try {
		...
		await page.waitForFunction(() => document.querySelectorAll('[data-yweb-text]').length >= 114);
		...
		const inventory = await page.evaluate(() => {
			...
			const kinds = nodes.reduce((groups, node) => {
				const key = node.dataset.ywebKind;
			...
			return {
				...
				ids: nodes.map((node) => ({ id: node.id, uid: node.dataset.ywebText, kind: node.dataset.ywebKind, tag: node.tagName })),
		...
		assert.equal(inventory.kinds.LinkButton, 2, `LinkButton数: ${inventory.kinds.LinkButton}`);
		...
		for (const entry of inventory.ids) {
			...
			assert.match(entry.id, /^yweb-\d+$/, `ObjectID形式: ${entry.id}`);
			assert.equal(entry.id, `yweb-${entry.uid}`, `IDとObjectID不一致: ${entry.id}`);
		}
		const semanticInput = page.locator('[data-yweb-kind="LineEdit"]');
		assert.equal(await semanticInput.getAttribute('aria-label'), 'Account email');
		assert.equal(await semanticInput.getAttribute('name'), 'email');
		assert.equal(await semanticInput.getAttribute('autocomplete'), 'email');
		assert.equal(await semanticInput.getAttribute('inputmode'), 'email');
		assert.equal(await semanticInput.getAttribute('aria-description'), 'Contact address');
		...
		// ButtonはGodotのControl矩形をnative操作範囲として所有する。
		...
		assert.equal(await page.locator('[data-yweb-kind="LineEdit"]').getAttribute('aria-label'), 'Account email', 'LineEditの名前なし');
		assert.equal(await page.locator('[data-yweb-kind="TextEdit"]').getAttribute('aria-label'), 'Comment', 'TextEditの名前なし');
		...
		assert.ok(Math.abs(buttonBefore.box.width - 230) <= 1 && Math.abs(buttonBefore.box.height - 54) <= 1, `Button操作範囲がControlと違う: ${JSON.stringify(buttonBefore.box)}`);
		assert.ok(buttonBefore.box.x >= 32 && buttonBefore.box.x + buttonBefore.box.width <= 263, 'Button操作範囲が背景外');
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'BUTTON MODEL:DOWN:1:0'));
		assert.equal((await item(page, 'THEME OVERRIDE')).color, 'rgb(255, 79, 154)', 'Button pressed色が未反映');
		await page.mouse.up();
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent.startsWith('BUTTON MODEL:UP:')));
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'THEME ACTIVE'));
		...
		// 押下時action modeはmouseup前に発火し、native pointer状態をGodotへ戻す。
		...
		await page.mouse.down();
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'PRESS FIRED'));
		await page.mouse.up();
		...
		const linkNode = page.locator(`#${linkBefore.id}`);
		assert.equal(await linkNode.getAttribute('href'), 'https://docs.godotengine.org/', 'LinkButton URI未反映');
		assert.equal(await linkNode.getAttribute('rel'), 'noopener noreferrer', '外部linkのrel不足');
		assert.equal(await page.locator('[data-yweb-kind="LinkButton"]', { hasText: 'UNSAFE LINK' }).getAttribute('href'), null, '本番LinkButtonへHTTP URLを許可した');
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'LINK PRESSED'));
		assert.equal((await item(page, 'LINK PRESSED')).id, linkBefore.id, 'LinkButtonのObjectIDが入力で変化');
		await linkNode.focus();
		assert.notEqual(await linkNode.evaluate((node) => getComputedStyle(node).outlineStyle), 'none', 'keyboard focus枠なし');
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'BUTTON MODEL:UP:0:1'));
		await linkNode.press('Enter');
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'LINK BUTTON'));
		assert.equal((await item(page, 'LINK BUTTON')).id, linkBefore.id, 'LinkButtonのkeyboard入力でObjectIDが変化');
		assert.equal(await page.locator('[data-yweb-sent]').count(), 0, '入力本文をDOM属性へ複製した');
		await linkNode.blur();
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'BUTTON MODEL:UP:0:0'));

		// composition中はDOMに保持し、確定時だけLineEditへ日本語と絵文字を戻す。
		const composingLineState = await page.evaluate(() => {
			const input = document.querySelector('[data-yweb-kind="LineEdit"]');
			input.focus();
			...
			return [...document.querySelectorAll('[data-yweb-text]')].find((node) => node.textContent.startsWith('LINE MODEL:'))?.textContent;
		});
		assert.ok(!composingLineState.includes('日本語'), `IME未確定値がGodotへ流入: ${composingLineState}`);
		await page.evaluate(() => document.querySelector('[data-yweb-kind="LineEdit"]').dispatchEvent(new CompositionEvent('compositionend', { bubbles: true, data: '日本語😀' })));
		await page.waitForTimeout(100);
		const lineResult = await page.evaluate(() => { const input = document.querySelector('[data-yweb-kind="LineEdit"]'); return { value: input.value, data: { ...input.dataset } }; });
		assert.equal((await page.locator('[data-yweb-text]', { hasText: 'LINE MODEL:' }).first().textContent()), 'LINE MODEL:日本語😀:4:4', `IME確定値がGodotへ届かない: ${JSON.stringify({ browserErrors, lineResult })}`);
		...
		await page.waitForFunction(() => document.querySelector('[data-yweb-kind="LineEdit"]')?.value === 'PROGRAMMATIC');
		await page.evaluate(() => {
			const input = document.querySelector('[data-yweb-kind="LineEdit"]');
			input.value = '日本語😀';
			input.setSelectionRange(input.value.length, input.value.length);
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'LINE MODEL:日本語😀:4:4'));
		await page.evaluate(() => {
			const input = document.querySelector('[data-yweb-kind="LineEdit"]');
			input.setSelectionRange(1, 3);
			input.dispatchEvent(new Event('select', { bubbles: true }));
		});
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'LINE MODEL:日本語😀:1:3'));
		...
		await page.waitForFunction((value) => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === `LINE MODEL:${value}:24:24`), limited);
		assert.equal(Array.from((await control(page, 'LineEdit')).value).length, 24, 'Unicode文字上限を超過');
		await page.locator('[data-yweb-kind="LineEdit"]').press('Enter');
		await page.waitForFunction(() => document.activeElement?.dataset.ywebKind !== 'LineEdit');
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent.startsWith('AREA MODEL:一行|二行😀:1:3:1:3:')));

		// textareaのnative scrollを唯一の表示scrollbarとし、Godotのscroll値は変えない。
		await page.evaluate(() => {
			const area = document.querySelector('[data-yweb-kind="TextEdit"]');
		...
		await page.waitForFunction(() => {
			const area = document.querySelector('[data-yweb-kind="TextEdit"]');
			return area.scrollTop > 0 && area.scrollLeft > 0;
		});
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent.startsWith('AREA MODEL:ROW0')));
		const scrollModel = await page.evaluate(() => [...document.querySelectorAll('[data-yweb-text]')].find((node) => node.textContent.startsWith('AREA MODEL:ROW0'))?.textContent || '');
		assert.match(scrollModel, /:0:0$/, `GodotがBrowser scrollを所有: ${scrollModel}`);
		...
		assert.match((await item(page, 'SCORE 00000'))?.id || (await page.locator('[data-yweb-text]', { hasText: 'SCORE ' }).first().getAttribute('id')), /^yweb-\d+$/, 'score未同期');

		// 解放済みObjectIDのDOMを一frame境界で回収する。
		await page.waitForFunction(() => ![...document.querySelectorAll('[data-yweb-text]')].some((node) => node.textContent === 'TEMPORARY DOM'));
		const hidden = await item(page, 'HIDDEN DOM');
		assert.ok(!hidden || hidden.display === 'none', '非表示Labelが表示');
