...
(async () => {
	...
	try {
		...
		for (const entry of inventory.ids) {
			...
			assert.match(entry.id, /^yweb-text-\d+$/, `ObjectID形式: ${entry.id}`);
			assert.equal(entry.id, `yweb-text-${entry.uid}`, `IDとObjectID不一致: ${entry.id}`);
		}
		...
		assert.equal(await page.locator('[data-yweb-kind="LinkButton"]').getAttribute('href'), 'https://docs.godotengine.org/', 'LinkButton URI未反映');
		...
		await page.locator('[data-yweb-kind="LinkButton"]').blur();
		...
		assert.match((await item(page, 'SCORE 00000'))?.id || (await page.locator('[data-yweb-text]', { hasText: 'SCORE ' }).first().getAttribute('id')), /^yweb-text-\d+$/, 'score未同期');
