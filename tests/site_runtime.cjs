...
async function main() {
	...
	container = child.execFileSync('docker', ['run', '--rm', '-d', '-p', `127.0.0.1:${port}:8080`, '-v', `${site}:/usr/share/nginx/html:ro`, '-v', `${path.join(site, 'nginx-yweb.conf.example')}:/etc/nginx/conf.d/default.conf:ro`, 'nginx:alpine'], { encoding: 'utf8' }).trim();
