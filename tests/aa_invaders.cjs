...
// 書き出したsiteを390x844で動かし、移動、命中、下降、敵弾、防壁損耗を観測する。
...
async function box(page, text) {
	return page.evaluate((value) => {
		const node = [...document.querySelectorAll('[data-yweb-text]')].find((item) => item.textContent === value);
		if (!node) return null;
...
async function main() {
	...
	await page.waitForFunction(() => [...document.querySelectorAll('[data-yweb-text]')].filter((node) => ['╱(• •)╲', '〈[°°]〉', 'Ψ(××)Ψ', '╭[••]╮', '≪(oo)≫'].includes(node.textContent)).length === 40);
	...
	const layout = await page.evaluate(() => {
		...
		const root = document.querySelector('#yweb-text-root').getBoundingClientRect();
		const nodes = [...document.querySelectorAll('[data-yweb-text]')].map((node) => node.getBoundingClientRect());
		return { canvas: { width: canvas.width, height: canvas.height }, canvasPixels: { width: canvasNode.width, height: canvasNode.height }, root: { width: root.width, height: root.height }, minLeft: Math.min(...nodes.map((rect) => rect.left)), maxRight: Math.max(...nodes.map((rect) => rect.right)), documentWidth: document.documentElement.scrollWidth, domCount: nodes.length, buttonCount: document.querySelectorAll('[data-yweb-kind="Button"]').length, webgl2: !!canvasNode.getContext('webgl2') };
	});
	...
	await page.waitForFunction(() => {
		const stats = [...document.querySelectorAll('[data-yweb-text]')].find((node) => node.textContent.startsWith('INV '))?.textContent || '';
	...
	const stats = await page.evaluate(() => [...document.querySelectorAll('[data-yweb-text]')].find((node) => node.textContent.startsWith('INV ')).textContent);
