...
async function measure(browser, kind, port) {
	...
	if (kind === 'minimum') {
		await page.waitForFunction(() => document.querySelectorAll('[data-yweb-text]').length >= 110);
...
async function capture(browser, kind, port) {
	...
	const masks = kind === 'minimum' ? await page.evaluate(() => [...document.querySelectorAll('[data-yweb-text]')]
