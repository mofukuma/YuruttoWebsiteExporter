...
(async () => {
	...
	try {
		...
		assert.match(godouStart.id, /^gdweb-text-\d+$/);
		assert.match(omochiStart.id, /^gdweb-text-\d+$/);
		assert.equal(await page.locator('[data-gdweb-kind="LinkButton"]').getAttribute('href'), 'https://godotengine.org/');
		assert.equal(await page.locator('[data-gdweb-kind="LinkButton"]').evaluate((node) => getComputedStyle(node).pointerEvents), 'none', 'LinkButtonがCanvas mouseを遮断');
		...
		while (Date.now() < deadline) {
			const falling = await page.locator('[data-gdweb-kind="Button"]').evaluateAll((nodes) => nodes.map((node) => {
				const box = node.getBoundingClientRect();
		...
		await page.waitForFunction(() => {
			const text = [...document.querySelectorAll('[data-gdweb-text]')].find((node) => /^投下 \d+ \/ フレーム \d+$/.test(node.textContent))?.textContent;
		...
		const state = await page.evaluate(() => {
			const frameText = [...document.querySelectorAll('[data-gdweb-text]')].find((node) => /^投下 \d+ \/ フレーム \d+$/.test(node.textContent)).textContent;
			const ids = [...document.querySelectorAll('[data-gdweb-kind="Button"]')].map((node) => node.id);
