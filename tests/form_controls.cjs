...
(async () => {
	...
	try {
		...
		await page.waitForFunction(() => document.querySelector('[data-yuruttoweb-kind="LineEdit"]') && document.querySelector('[data-yuruttoweb-kind="TextEdit"]'));

		const inventory = await page.evaluate(() => {
			const nodes = [...document.querySelectorAll('[data-yuruttoweb-text]')];
			return {
				config: globalThis.YURUTTOWEB_TEXT_CONFIG,
				texts: nodes.map((node) => node.textContent || node.value),
				kinds: nodes.reduce((map, node) => ({ ...map, [node.dataset.yuruttowebKind]: (map[node.dataset.yuruttowebKind] || 0) + 1 }), {}),
				forms: nodes.filter((node) => ['INPUT', 'TEXTAREA'].includes(node.tagName)).map((node) => ({ tag: node.tagName, kind: node.dataset.yuruttowebKind, placeholder: node.placeholder, font: getComputedStyle(node).fontFamily, marked: node.hasAttribute('data-yuruttoweb-text') })),
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yuruttoweb-kind="Label"]')].some((node) => node.textContent === 'LINE:日本語😀入力:6:1:日本語😀入力'));
		await compose(page, 'textarea[placeholder="複数行の日本語を入力"]', '一行目\n二行目');
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yuruttoweb-kind="Label"]')].some((node) => node.textContent === 'AREA:一行目|二行目:1:3'));
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yuruttoweb-kind="Label"]')].some((node) => node.textContent === 'LINE::0:2:'));
		await compose(page, 'input[placeholder="日本語を入力"]', '消去対象');
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yuruttoweb-kind="Label"]')].some((node) => node.textContent === 'LINE:消去対象:4:3:消去対象'));
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yuruttoweb-kind="Label"]')].some((node) => node.textContent === 'LINE::0:4:'));

		// Canvas経由のButton操作でThemeを変更し、同じ複数項目DOMへ反映する。
		const tabBefore = await page.evaluate(() => {
			const node = [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].find((item) => item.textContent === 'ホーム');
		...
		const itemBefore = await page.evaluate(() => {
			const node = [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].find((item) => item.textContent === 'りんご');
			return { id: node.id, scale: Number(node.dataset.yuruttowebTextScale) };
		});
		await page.evaluate(() => {
			const style = document.createElement('style');
			style.textContent = '@font-face{font-family:YuruttoWeb-Delayed;src:url("/delayed-font.woff2") format("woff2");font-display:block}';
			document.head.appendChild(style);
			globalThis.YURUTTOWEB_FONT_MAP = new Proxy({}, { get: () => ({ family: 'YuruttoWeb-Delayed' }) });
		});
		...
		await page.waitForFunction(() => [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].some((node) => node.textContent === 'WWWWWWWW'));
		await page.waitForFunction(() => document.fonts.status === 'loading');
		const itemLoading = await page.evaluate(() => {
			const node = [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].find((item) => item.textContent === 'WWWWWWWW');
			return { scale: Number(node.dataset.yuruttowebTextScale), local: Number.parseFloat(node.style.width) };
		});
		await page.waitForFunction(() => document.fonts.check('15px YuruttoWeb-Delayed', 'WWWWWWWW'));
		const itemAfter = await page.evaluate(() => {
			const node = [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].find((item) => item.textContent === 'WWWWWWWW');
			return { id: node.id, scale: Number(node.dataset.yuruttowebTextScale), width: node.scrollWidth, local: Number.parseFloat(node.style.width) };
		});
		...
		await page.evaluate(() => {
			const style = document.createElement('style');
			style.textContent = '@font-face{font-family:YuruttoWeb-Missing;src:url("/missing-font.woff2") format("woff2")}';
			document.head.appendChild(style);
			globalThis.YURUTTOWEB_FONT_MAP = new Proxy({}, { get: () => ({ family: 'YuruttoWeb-Missing' }) });
		});
		await page.mouse.click(70, 380);
		await page.waitForFunction(() => document.fonts.status === 'loaded');
		await page.evaluate(() => new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))));
		const missingFont = await page.evaluate(() => {
			const node = [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].find((item) => item.textContent === 'おもち');
			return { dom: !!node, scale: Number(node?.dataset.yuruttowebTextScale) };
		});
		...
		const controlRects = await page.evaluate(() => [...document.querySelectorAll('[data-yuruttoweb-kind="ControlText"]')].map((node) => {
		...
		await page.evaluate(() => { globalThis.YURUTTOWEB_TEXT_CONFIG.avoidCanvasThemeFont = false; });
		await page.setViewportSize({ width: 389, height: 844 });
		await page.setViewportSize({ width: 390, height: 844 });
		await page.waitForFunction(() => ![...document.querySelectorAll('[data-yuruttoweb-text]')].some((node) => node.textContent === '省略表示のDOM代替文字'));
		await page.evaluate(() => new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve))));
		assert.match(fs.readFileSync(path.join(__dirname, '../build/overlay/platform/web/yuruttoweb_text_sync.cpp'), 'utf8'), /簡易DOM表示へ置き換えます/, '代替warningなし');
