...
archive=$repo/tmp/godot-$GODOT_VERSION.tar.xz # Godot公式source archive。
source_root=$repo/tmp/godot-minimum-source # overlay適用済みsource。
emsdk=$repo/tmp/emsdk # 固定Emscripten SDK。
mkdir -p "$repo/tmp"
work=$(mktemp -d "$repo/tmp/runtime-source.XXXXXX") # source展開用一時領域。
trap 'rm -rf "$work"' EXIT

# lock、patch、overlayを一つのsource識別値へまとめる。
stamp=$(cd "$repo" && {
	shasum -a 256 build/source.lock build/patches/web_gdweb_text.patch
...
if test -e "$source_root"; then
	current=$(cat "$source_root/.gdweb-source-stamp" 2>/dev/null || true)
